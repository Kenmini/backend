...
| `next_step_hint` | string \| null | 次のアクション提案。easyパスでは `null`（advancedパスで生成） |
...
| `confidence` | number | 0.0〜1.0。検索の類似度スコア（丸め済み） |
...
検索スコアが `GAP_THRESHOLD`（デフォルト `0.4`）を下回った場合、バックエンドは回答を生成しません。正直なメッセージを返し、`is_gap: true` を設定し、`citations` を空にして、質問を教授向けに記録します。
