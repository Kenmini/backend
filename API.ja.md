...
| `visual_data` | object \| null | 検索されたPDFページ画像、または静的な図・ホットスポット情報 |
...
| `visual_data.image_url` | string \| null | 最上位の根拠付きPDFページをJPEG化したdata URL |
| `visual_data.source` | string \| null | 出典PDFのファイル名 |
| `visual_data.page_number` | integer \| null | 出典PDFの1始まりページ番号 |
| `visual_data.caption` | string \| null | ページに対応する検索テキスト |
...
`image_url` がある場合、フロントエンドはその値を画像の `src` に直接指定できます。PDFの取得・描画に失敗しても、回答は維持され、追加フィールドだけが `null` になります。
